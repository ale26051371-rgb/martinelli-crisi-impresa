...
function buildTransitionOverlay() {
  return `<div class="page-transition active" id="pageTransition">
  <div class="transition-content">
    <div class="transition-num" id="transitionNum"></div>
...
function navigateToPage(href) {
  ...
  tl.to(overlay, { y: 0, duration: 0.7, ease: 'power3.inOut' })
    ...
    .add(() => {}, '+=0.3'); // brief pause then navigate
}
...
function exitTransition() {
  ...
  // Check if we should animate out (came from another page)
  ...
  const tl = gsap.timeline();
  tl.to(content, { opacity: 0, y: -30, duration: 0.3, ease: 'power2.in', delay: 0.15 })
    .set(line, { scaleX: 0 })
    .to(overlay, { y: '-100%', duration: 0.7, ease: 'power3.inOut' }, '-=0.1')
    .set(overlay, { display: 'none', onComplete: () => overlay.classList.remove('active') });
}
